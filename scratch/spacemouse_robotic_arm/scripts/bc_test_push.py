...
from training_code_push import FlattenGoalEnv # Import data from demos
...
def evaluate_success_rate(policy, env_id="FrankaPushSparse-v0", num_episodes=5):
    ...
    # Use max_episode_steps=150 to fail faster on bad runs and speed up evaluation
...
def train_on_operator_data(data_path="operator_data_push.pkl", total_epochs=40, eval_freq=5, eval_episodes=5):
    ...
    for traj in raw_trajectories:
        # Filter out junk/cancelled trials that are too short
        if len(traj["acts"]) < 5:
            continue
    ...
    # Evaluate at Epoch 0 (Before Training) to get a baseline
    ...
    print(f"Epoch  0/{total_epochs:2d} | Success Rate: {success_rate_initial:5.1f}% (Untrained)")
...
if __name__ == "__main__":
    # You can tweak total_epochs and eval_freq here
    train_on_operator_data(data_path="operator_data_push.pkl", total_epochs=40, eval_freq=5, eval_episodes=5)